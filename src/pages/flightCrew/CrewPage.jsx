...
export function CrewPage() {
  ...
  const myFlights = flights.filter((flight) => flight.crewAssigned.includes(currentUser.id))
  ...
  return (
    <>
      <PageHeader
        actions={
          <>
            ...
            <ActionButton onClick={() => data.confirmAssignment(nextFlight.id, currentUser.id)}>Confirm assignment</ActionButton>
          </>
        }
      ...
      <section className="metrics-grid crew-metrics">
        ...
        <MetricCard icon={ShieldCheck} label="Qualifications" value="Valid" detail={currentUser.qualification} tone="green" />
      </section>
      ...
      <Panel title="My flight duties">
        <DataTable
          columns={[
            ...
            { key: 'action', label: 'Action', render: (row) => <ActionButton tone="secondary" onClick={() => data.confirmAssignment(row.id, currentUser.id)}>Confirm</ActionButton> },
          ]}
...
function FlightDetails({ currentUser, data, flight }) {
  return (
    <>
      <PageHeader actions={<ActionButton onClick={() => data.confirmAssignment(flight.id, currentUser.id)}>Confirm assignment</ActionButton>} eyebrow="Flight details" title={`${flight.id} ${flight.route}`} />
      <section className="dashboard-grid">
        ...
        <Panel title={currentUser.position.includes('Cabin') || currentUser.position === 'Purser' ? 'Cabin briefing' : 'Flight deck briefing'}>
...
function Requests({ currentUser, data, openForm }) {
  const mine = data.requests.filter((request) => request.employeeId === currentUser.id)
  return (
    <>
    ...
    <>
      ...
      <Panel title="Active documents">
        <div className="settings-grid">
          <div><strong>Position</strong><span>{currentUser.position}</span></div>
          <div><strong>Aircraft qualification</strong><span>{currentUser.qualification}</span></div>
          <div><strong>Medical / recurrent</strong><span>{currentUser.medical}</span></div>
        </div>
      </Panel>
    </>
  )
...
function Documents({ currentUser }) {
  const pilot = ['Captain', 'First Officer'].includes(currentUser.position)
  return (
    <>
    ...
    <>
      ...
      <Panel title="Security details">
        <div className="settings-grid">
          ...
          <div><strong>Last seen</strong><span>{currentUser.lastSeen}</span></div>
          <div><strong>Active session</strong><span>Current browser local session</span></div>
        </div>
      </Panel>
    </>
  )
